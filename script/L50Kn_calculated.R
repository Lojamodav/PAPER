...
## L50 is calculated for both sexs
...
## L50 is calculated for male


Male<-subset(Hake,Sex=="Male")

L50male = gonad_mature(Male, varNames = c("Length", "Maturity"), inmName = "Inmature",
...
## L50 is calculated for female

Female<-subset(Hake,Sex=="Female")
...
lines(Female~x,type="l",lwd=2,col="red")
legend("bottomright",c("Male","Female"),lwd=2,col=c("black","red"))


#######  L50 is calculated for each sex and for each Year from 1982 to 2019 ##########
...
L5085m<-subset(Hake,Year=="1985" & Sex=="Male")
...
L5085f<-subset(Hake,Year=="1985" & Sex=="Female")
...
plot(L5016f, xlab = " Length (cm)", ylab = "Proportion of mature female", col = c("blue", "red"),onlyOgive = TRUE)
...
plot(L5017h, xlab = "Length (cm)", ylab = "Proportion of mature", col = c("blue", "red"), onlyOgive = TRUE)
...
plot(L5017m, xlab = " Length (cm)", ylab = "Proportion of mature male", col = c("blue", "red"),onlyOgive = TRUE)
...
L5019h=subset(datos, Year=="2019")

L5019h = gonad_mature(L5019h, varNames = c("Length", "Maturity"), inmName = "Inmature",
                     matName = "Mature", method = "fq", niter = 999)

plot(L5019h, xlab = "Length (cm)", ylab = "Proportion of mature", col = c("blue", "red"), onlyOgive = TRUE)
...
L5019f<-subset(Hake,Year=="2019" & Sex=="Female")

L5019f = gonad_mature(L5019f, varNames = c("Length", "Maturity"), inmName = "Inmaduro",
                      matName = "Mature", method = "fq", niter = 999)

print(L5019f)

plot(L5019f, xlab = " Length (cm)", ylab = "Proportion of mature female", col = c("blue", "red"),onlyOgive = TRUE)


#### Kn was calculated with total weight for each sex and for each year ####

Haketw <- Hake %>%
  filterD(!is.na(Wtotal),!is.na(Length)) %>%
  mutate(logL=log(Length),logW=log(Wtotal))
...
plot(Haketw$logW~Haketw$logL, xlab="(Log) weight ", ylab="(Log) Length")
abline(lm1, col=2)

## Kn is calculated
Haketw %>% mutate(lwresid=residuals(lm1))
Haketw<-Haketw %>% mutate(predW=fitted(lm1)*100,Kn=Wtotal/predW)

## Kn for each sex 

Sex<-with(Haketw,tapply(Haketw$Kn, list(Haketw$Year, Haketw$Sex), mean))
Knm<-Sex[,1];Knm
Knf<-Sex[,2];Knf
...
#Missing values were detected in the L50 and Kn time series for both sexes, males and females.
#Now, the graphs of Kn and L50 for each sex with missing values are shown below.
